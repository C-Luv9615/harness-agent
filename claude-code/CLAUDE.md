...
每次新会话开始，先使用 `session-handoff` skill：通过 `.harness/` 和 `.specify/` 文件状态机械判断是接力还是全新开始，恢复上下文后报告状态，用户未明确说"新的"则默认接力。
...
使用 `vela-harness` skill：编译 → 启动 → 执行测试 → 解析 cmocka 输出。FAIL 则分析→修复→重验证，卡住规则生效。
...
- `.specify/`：需安装 mispec CLI（`pip install mispec`），执行 `mispec init . --ai claude --force --no-git`
- `.gitignore`：确保包含 `.specify/`
