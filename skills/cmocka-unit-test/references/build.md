...
请选择要编译的 lunch config：
  ...
  1. vendor/template/boards/chip_name/board_name/configs/nsh
  2. vendor/template/boards/chip_name/board_name/configs/ap

请选择编号，或手动输入其他 lunch config 路径（如 vendor/<vendor>/boards/<chip>/<board>/configs/<target>）：
```
...
例如用户输入 `vendor/template/boards/chip_name/board_name/configs/nsh`，则 defconfig 路径为：
...
CONFIG_CM_[MODULE_UPPER]_TEST=y
```

以及 Step 2a 中发现的所有依赖配置项（如果 defconfig 中尚未启用）。

其中 `[MODULE_UPPER]` 为本次生成用例对应的模块名大写形式（与 Kconfig 中的 `config CM_[MODULE_UPPER]_TEST` 一致）。
...
3. 执行 `m -j10` 前，先检查 out 目录下的 `.config` 是否包含本次新增的配置项。

   out 目录规则：将 `[lunch_config_path]` 最后三段路径（vendor/board/config）用 `_` 连接，例如 `vendor/template/boards/chip_name/board_name/configs/nsh` → `out/template_board_name_nsh`。

   检查命令：
   ```bash
   grep "CONFIG_CM_[MODULE_UPPER]_TEST" out/[out_dir]/.config
   ```

   - **包含**：直接执行 `m -j10`
   - **不包含**：先执行 `m distclean`，再执行 `m -j10`
...
第四条命令设置 `ignoreWarning: true`，`timeout: 3600000`（60 分钟），将输出重定向到 `build.log`（覆盖写入，不追加）。
...
1. 判断是否由本次生成的代码引入（文件路径包含本次生成的测试目录 `tests/velatest/[module]/`）：
